...
use test_network::{Network, TestRuntime};
...
async fn setup_bench_network(maximum_transactions_in_block: u32) -> Client {
    let (_, mut client) = <Network>::start_test(4, maximum_transactions_in_block).await;
...
fn overflow_4_peers(maximum_transactions_in_block: u32) {
    let rt = Runtime::test();
    let mut iroha_client = rt.block_on(setup_bench_network(maximum_transactions_in_block));
